...
<body>
	...
	<div class="col-md-12">
		<div class="panel z-depth-1">
			<div class="panel-content">
				<form id="form1" action="/story/update" method="post">
				<div class="row">
					<input type="hidden" id="tour_id" name="tour_id" value="${tour_id}">
					<input type="hidden" id="story_id" name="story_id" value="${story_id}">
					<input type="hidden" id="day_date" name="day_date" value="${dayAndDateOther}">
					<input type="hidden" id="place_name" name="place_name" value="${placeName}">
					<div class="col-xs-12 col-sm-7 col-md-7" style="margin: auto;">
						...
						<select class="form-control boxed-input" id="tourDate" onChange="itemChange(${tour_id })" style="width: 200px; margin-bottom: 10px">
							...
							<c:forEach var="schedulePlan" items="${schedulePlans }">
								<c:if test="${schedulePlan.tour_date eq dayAndDateOther}">
									<option selected value="${schedulePlan.tour_date }">day${i=i+1} (${schedulePlan.tour_date })</option>
								</c:if>
								<c:if test="${schedulePlan.tour_date ne dayAndDate}">
						...
						<div class="form-group" style="margin-bottom: 30px">
							<div>
								<input type="text" class="form-control boxed-input" name="title" value="${story.title }">
							</div>
						</div>

						<textarea class="form-control boxed-input" rows="3" name="content" style="margin-bottom: 30px; height:300px">${story.content }</textarea>
						...
						<div id="fileContainer">
							<c:forEach var="storyImage" items="${story.storyImageList }">
								<div id="fileUploadDiv">
									<div id="uploadedImg"><img src="/resources/upload${storyImage.image }" style="width:100%;height:100%"></div>
									<div id="fileDelBtnDiv"><a data-src="${storyImage.image }" style="color:#BDBDBD">X</a></div>
								</div>
							</c:forEach>
						</div>
						...
						<div class="col-md-12" style="text-align: right; margin-bottom: 30px;">
							...
							<button class="btn btn-secundary" type="button" name="action" style="margin-right: 5px;">취소</button>
						</div>
	...
	$(document).ready(function(){
		getPlace($("#tour_id").val(), $("#day_date").val());
		deleteFiles();
	});
	...
	function getPlace(tour_id, selectedDate){
		$.getJSON("/story/getPlace/"+tour_id+"/"+selectedDate, function(data){
			var option = "";
			
			 $.each(data, function(key,state) {
				if(state.name == $("#place_name").val()){
					option += "<option selected value='"+key+"'>"+state.name+"</option>"
				}else{
					option += "<option value='"+key+"'>"+state.name+"</option>"
				}
			});
	...
	function deleteFiles(){
		...
		if(arr.length > 0){
			$.post("/story/deleteAllFiles", {files:arr}, function(){
