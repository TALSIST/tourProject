...
<script>
	function classact() {
		...
		$('.spot_to_inspot').click(function(){
			...
			console.log(name + "" + sub_category_id); 		
			$.ajax({
				type:'POST',
				url:'/addSpot',
				data:{"sub_category_id" : sub_category_id,	"name":name},
				success:function(response){
					 alert(JSON.stringify(response));
					var tag='${sub_category_name}'; //공란으로 뜸.
					//var sub="<input type=hidden value="+response[0].name+">";
				 ...
				 success:function(response){
					 ...
					 for(var i=0; i<response.length;i++){
						 ...
						 "<div class=\"fl info_box\">"+
						 	...
						 	"<div class=\"sub include_pn_day\" style=\"color:#1a7ad9\"></div>"+
						 "</div>"+
						 "<div class=\"spot_to_inspot\" sub-category-id="+response[i].sub_category_id+" data-name="+response[i].name+"><img style=\"width:20px; height:20px;\"src=\"/resources/img/plan/addspot.png \"></div><div class=\"clear\"></div></div>";
					 }
					 $('#locList').html(""+sub+"");
					 classact();
				 }
